	...
	return new DbTableConfig()
	{
		...
		ProcessTable = new()
		{
			...
			ColumnDefinitions = new()
			{
				...
				new ColumnDefinition() { ColumnName = "destination_table_name", TypeName = "text" },
				new ColumnDefinition() { ColumnName = "datasource_name", TypeName = "text" },
				new ColumnDefinition() { ColumnName = "datasource_key", TypeName = "text" },
		...
		DatasourceKeyColumn = "datasource_key",
...
DbDestination GetDestination()
{
	return new DbDestination()
	{
		Table = new()
		{
			...
			Columns = new() { "sale_journal_id", "journal_closing_date", "sale_date", "shop_id", "price" }
		},
		Sequence = new()
		{
...
ExecuteSql(dbConnectionConfig, "insert into sale_journal_flip_requests(sale_journal_id) select sale_journal_id from sale_journals");

var flipTransfer = new FlipTransferBatch(environment, logger);
flipTransfer.Execute(datasource.Destination);
