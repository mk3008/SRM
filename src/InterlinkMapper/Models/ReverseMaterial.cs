...
public class ReverseMaterial : DatasourceMaterial
{
	...
	internal void ExecuteTransfer(IDbConnection connection)
	{
		...
		{
			var m = ToDatasourceReverseMaterial(datasource);
	...
	private SelectQuery CreateSelectQuery(InterlinkDatasource datasource)
	{
		var source = ObjectRelationMapper.FindFirst<InterlinkDatasource>();

		var sq = new SelectQuery();
		sq.AddComment("filterd by datasource");
	...
	private DatasourceReverseMaterial ToDatasourceReverseMaterial(InterlinkDatasource datasource)
	{
		var sq = CreateSelectQuery(datasource);
